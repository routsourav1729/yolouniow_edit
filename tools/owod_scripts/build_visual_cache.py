"""Build a per-FPN-level (n_novel, K_l, D) post-BN visual cache from K-shot GT
boxes. Each FPN level has its own BNContrastiveHead (own BN affine) — features
across levels live in different spaces, so we cache and consume per-level.

For each novel GT and each FPN level: take the best-IoU anchor at that level
(if IoU >= --iou-thresh) and store its post-BN cls_embed. A single GT may thus
contribute one support per level.
"""
import argparse
import os
import sys
...
def main():
    ...
    assert not model.training, 'BUG: model must be in eval() for cache build.'
    head = model.bbox_head
    hm = head.head_module
    num_classes = hm.num_classes
    num_levels = hm.num_levels

    n_base = model.num_prev_classes
    n_novel = num_classes - n_base - 2
    embed_dim = hm.embed_dims
    print(f'[init] n_base={n_base}  n_novel={n_novel}  '
          f'embed_dim={embed_dim}  num_levels={num_levels}')
    ...
    # cache_lists[lvl][novel_idx] -> list of (D,) tensors.
    cache_lists = [defaultdict(list) for _ in range(num_levels)]
    ...
    with torch.no_grad():
        for i in range(len(train_ds)):
            ...
            img_feats, _ = model.extract_feat(img, [ds])
            assert len(img_feats) == num_levels

            # Per-level: BN feature, decoded anchor boxes.
            # grid_priors requires all levels at once.
            featmap_sizes = [(f.shape[-2], f.shape[-1]) for f in img_feats]
            all_priors = head.prior_generator.grid_priors(
            ...
            for lvl in range(num_levels):
                ...
                bn_embed = cls_contrast.norm(cls_embed)               # (1,D,H,W)
                ...
                priors = all_priors[lvl]
                flat_bbox = bbox_pred.permute(0, 2, 3, 1).reshape(1, -1, 4)
                anchor_boxes = head.bbox_coder.decode(
                    priors[..., :2], flat_bbox, priors[:, [2]][..., 0])[0]
                flat_bn = bn_embed.permute(0, 2, 3, 1).reshape(
                    -1, embed_dim)                                    # (N_l, D)
                level_data.append((flat_bn, anchor_boxes))

            # GT boxes in padded-image space.
            ...
            # Per-level matching: each GT may contribute up to one support per
            # level (whichever level has IoU >= thresh).
            for lvl, (flat_bn, anchor_boxes) in enumerate(level_data):
                ious = box_iou(gt_boxes, anchor_boxes)                # (G, N_l)
                best_iou, best_idx = ious.max(dim=1)
                for gi in range(len(gt_cls)):
                    if float(best_iou[gi]) < args.iou_thresh:
                        continue
                    cache_lists[lvl][gt_cls[gi]].append(
                        flat_bn[best_idx[gi]].clone().cpu())

            if (i + 1) % 25 == 0 or (i + 1) == len(train_ds):
                print(f'  processed {i + 1}/{len(train_ds)}')

    # Stack per-level per-class.
    cache_per_level = {}
    matched = {lvl: {} for lvl in range(num_levels)}
    for lvl in range(num_levels):
        level_dict = {}
        for ci in range(n_novel):
            feats = cache_lists[lvl][ci]
            matched[lvl][ci] = len(feats)
            if feats:
                level_dict[ci] = torch.stack(feats, dim=0)
        cache_per_level[lvl] = level_dict

    # Per-level summary.
    print('\n[cache] per-level per-class match counts (level: P3, P4, P5):')
    print(f'  {"":35s}  ' + '  '.join(f'L{l}' for l in range(num_levels)))
    for ci in range(n_novel):
        counts = '  '.join(f'{matched[l][ci]:2d}' for l in range(num_levels))
        print(f'  [{ci:2d}] {novel_names[ci]:30s}  {counts}')

    print('\n[cache] within-class within-level mean cosine (sanity, want >0.5):')
    for lvl in range(num_levels):
        for ci, feats in cache_per_level[lvl].items():
            n = feats.shape[0]
            if n < 2:
                continue
            v = torch.nn.functional.normalize(feats, dim=-1)
            sims = [float((v[a] * v[b]).sum())
                    for a, b in combinations(range(n), 2)]
            m = float(np.mean(sims))
            warn = '  <-- LOW' if m < 0.5 else ''
            print(f'  L{lvl} [{ci:2d}] {novel_names[ci]:25s}  N={n}  '
                  f'within={m:.3f}{warn}')

    print('\n[cache] within-level cross-class mean cosine '
          '(within < cross flagged):')
    for lvl in range(num_levels):
        means = {ci: torch.nn.functional.normalize(f, dim=-1).mean(dim=0)
                 for ci, f in cache_per_level[lvl].items()}
        keys = sorted(means.keys())
        for i, a in enumerate(keys):
            for b in keys[i + 1:]:
                cs = float(torch.nn.functional.cosine_similarity(
                    means[a].unsqueeze(0), means[b].unsqueeze(0)))
                print(f'  L{lvl} ({novel_names[a]:20s}, {novel_names[b]:20s})  '
                      f'cross={cs:.3f}')

    torch.save({
        'cache_per_level': cache_per_level,
        ...
        'matched_per_class_per_level': matched,
    }, args.out)
    print(f'\n[write] {args.out}')
